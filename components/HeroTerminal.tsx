...
export default function HeroTerminal() {
  ...
  const [out, setOut] = useState<string[]>(["Type `help` for commands."]);
  ...
  function run(c: string) {
    ...
    if (cmd === "help") setOut(prev => [...prev, "Available: help, projects, contact, clear"]);
    else if (cmd === "projects") scrollToId("projects");
    else if (cmd === "contact") scrollToId("contact");
    else if (cmd === "clear") setOut([]);
    else setOut(prev => [...prev, "Command not found"]);
  }
          ...
          <div className="grid gap-6 px-4 py-6 md:grid-cols-2 md:gap-10 md:px-8 md:py-10">
            ...
            <div>
              <h1 className="text-3xl font-semibold tracking-tight text-white sm:text-4xl md:text-5xl">
                Build fast. Ship clean. <span className="text-emerald-400">Learn along the way.</span>
              </h1>
              <p className="mt-3 max-w-xl text-gray-400">
                Learning full-stack development through hands-on projects—building with modern tools, clean code, and attention to detail.
              </p>
              <div className="mt-6 flex flex-wrap items-center gap-3">
                <a href="#projects" className="group inline-flex items-center gap-2 rounded-lg border border-emerald-500/30 bg-emerald-500/10 px-4 py-2 text-sm font-medium text-emerald-300 hover:bg-emerald-500/20">
                  <FolderGit2 className="h-4 w-4" /> View work
            ...
            <div className="relative">
              ...
              <div className="overflow-hidden rounded-xl border border-white/10 bg-black/70">
                <div className="flex items-center gap-2 border-b border-white/10 px-3 py-2 text-xs text-gray-400">
                  ...
                  <span className="ml-2 hidden sm:inline text-gray-500">(try: help, projects, contact)</span>
                </div>
                ...
                <form onSubmit={(e) => { e.preventDefault(); const fd = new FormData(e.currentTarget); const cmd = String(fd.get("cmd") || ""); (e.target as HTMLFormElement).reset(); run(cmd); }}
                  ...
                  <input name="cmd" placeholder="Type a command…" className="w-full bg-transparent text-sm text-white placeholder:text-gray-500 focus:outline-none" />
                  <button type="submit" className="rounded-md border border-emerald-500/30 bg-emerald-500/10 px-3 py-1.5 text-xs text-emerald-300 hover:bg-emerald-500/20">Run</button>
                </form>
              </div>
            </div>
